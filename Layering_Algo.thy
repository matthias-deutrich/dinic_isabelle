...
definition ebfs_phase_invar :: "node \<Rightarrow> nat \<Rightarrow> _ graph \<Rightarrow> node set \<Rightarrow> (_ graph \<times> node set) \<Rightarrow> bool" where
  "ebfs_phase_invar s n c\<^sub>i Q \<equiv> \<lambda>(c', Q').
    ...
    \<and> Q' = exactDistNodes (Suc n) s \<inter> Graph.V c'"
...
proof (intro case_prodI conjI)
  ...
  (* TODO improve by introducing distance bounded locale for Bounded_S_SPU*)
  have "\<And>u. u \<in> V' \<Longrightarrow> min_dist s u \<le> n" using min_dist_transfer obtain_close_ST by auto
  then show "{} = exactDistNodes (Suc n) s \<inter> Graph.V c'" unfolding exactDistNodes_def by fastforce
  show "CapacityCompatibleGraphs c' c" by intro_locales
  show "Graph.E c' = Graph.E c' \<union> (exactDistNodes n s - exactDistNodes n s) \<times> {}" by simp
qed
...
proof
  from INVAR have "CapacityCompatibleGraphs c' c"
    and E': "Graph.E c' = Graph.E c\<^sub>i \<union> exactDistNodes n s \<times> Q'"
    and Q': "Q' = exactDistNodes (Suc n) s \<inter> Graph.V c'"
  ...
  proof
    (*from BSPU interpret Bounded_S_Shortest_Path_Union c\<^sub>i c s V n .*)

  qed
  then have "exactDistNodes (Suc n) s \<subseteq> Graph.V c'" unfolding exactDistNodes_def sorry
  then show "Q' = exactDistNodes (Suc n) s" using INVAR unfolding ebfs_phase_invar_def by fast
qed
...
lemma ebfs_phase_correct:
  ...
  assumes FIN: "finite (reachableNodes s)" (*"finite (exactDistNodes n s)"*)
    and BSPU: "Bounded_S_Shortest_Path_Union c' c s V n"
